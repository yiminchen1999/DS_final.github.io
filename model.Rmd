...
In this part, we were interested in the frequency of Squirrel recorded for each of the observation by primary fur color. 
...
dates %>% 
  ...
  geom_text(aes(y = n, label = age), color = "black", size=4) +
...
dates %>%
  ...
  count() %>% 
  ggplot(aes(x="", y = n, fill = primary_fur_color)) +
  geom_bar(stat="identity", width=1, color="white") +
  facet_grid(~age) + 
  coord_polar("y", start=0) +
  theme_void() + 
...
pie(x = dates1$n, labels = dates1$primary_fur_color, main = "Squirrels Distribution by Fur color") +
